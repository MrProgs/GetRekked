...
public class ReadReviewPresenter {
	...
	public void readReview(User reviewer){
		if(intent.get("Category").equals("Book")){
			...
			reviewServiceProxy.getBookReview(reviewer.getUserName(),book.getIsbn(),callbackReadBookReview);
		}
			...
			reviewServiceProxy.getMovieReview(reviewer.getUserName(),movie.getTitle(),movie.getRelease_date(),callbackReadMovieReview);
		}
			...
			reviewServiceProxy.getGameReview(reviewer.getUserName(), barcode, callbackReadGameReview);
		}
		this.intent.put("Action", "home");
		AppController.instance().go(intent);
	}
	...
	public void readAllReviews(String[] identifiers){
		if(intent.get("Category").equals("Book")){
			...
			System.out.println(book.getTitle());
			reviewServiceProxy.getAllBookReviews(book.getIsbn(),callbackReadBookReview);
		}
		else if (intent.get("Category").equals("Movie")){
			...
			System.out.println(movie.getTitle()+" ("+movie.getRelease_date()+")");
			reviewServiceProxy.getAllMovieReviews(movie.getTitle(),movie.getRelease_date(),callbackReadMovieReview);
		}
		else{ //(intent.get("Category").equals("Game"))
	...
	public void getContentInfo(){
		if(intent.get("Category").equals("Book")){
			...
			try {
				release_date = new SimpleDateFormat("MM/dd/YY", Locale.ENGLISH).parse(this.intent.get("Release_date"));
	...
	/*public void getUserInfo(){
		AsyncCallback<User> callbackGetUser = new AsyncCallback<User>() {
			...
			public void onSuccess(User x) {
				((ReadReviewView) view).setUserData(x);
				view.display();
			}
		};
